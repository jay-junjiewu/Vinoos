...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
      ...
      {projectImages.length > 0 && (
         <DialogContent 
           className="p-1 sm:p-2 md:p-4 border-0 max-w-none w-screen h-screen bg-black/90 backdrop-blur-sm flex items-center justify-center overflow-hidden [&>button[data-state=open]]:bg-transparent [&>button[data-state=open]]:text-white [&>button[data-state=open]]:hover:bg-white/10 [&>button[data-state=open]]:hover:text-white [&>button[data-state=open]]:focus:ring-white"
